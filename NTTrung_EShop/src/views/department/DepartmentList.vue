...
const setColumnTable = () => {
    columnsTable.value = [
        {
            title: MISAResource[resource.langCode]?.Manage?.DepartmentInfo?.DepartmentCode,
            dataIndex: 'id',
            key: 'DepartmentCode',
            width: '170',
            isShow: true,
        ...
        {
            title: MISAResource[resource.langCode]?.Manage?.DepartmentInfo?.DepartmentName,
            dataIndex: 'name',
            key: 'DepartmentName',
            width: '200',
            isShow: true,
        ...
        {
            title: MISAResource[resource.langCode]?.Manage?.DepartmentInfo?.Description,
...
const showModalAddData = () => {
    modalForm.setAction(MISAResource[resource.langCode]?.FormTitle?.Department?.Add);
...
const handleShowEditInfo = (data) => {
    modalForm.open();
    modalForm.setAction(MISAResource[resource.langCode]?.FormTitle?.Department?.Update);
...
const selectRowTable = (isChecked, data) => {
    try {
        dataTable.value = dataTable.value.map((d) => {
            if (d.DepartmentId === data.DepartmentId) {
...
async function getData() {
    try {
        ...
        const response = await departmentApi.request('filter');
...
const openDialogDeleteSelected = () => {
    dialog.open({
        title: MISAResource[resource.langCode]?.Dialog?.DeleteDepartment?.Title,
        content: MISAResource[resource.langCode]?.Dialog?.DeleteDepartment?.DeleteListContent,
...
const deleteOneData = async () => {
    await departmentApi.request(`${objectData.value.DepartmentId}`);
...
const deleteDataSelected = async () => {
    const listIds = dataSelected.value.map((data) => data.DepartmentId).join(',');
    idsDelete.value = listIds.split(',');
    departmentApi.method = Enum.ApiMethod.DELETE;
    departmentApi.data = listIds;
    await departmentApi.request('DeleteMany');
    handleUnCheckedAll();
    //Nếu xóa ở trang cuối hết thì gọi api về trang cuối - 1
...
const handleDeleteData = () => {
    try {
        ...
        dialog.open({
            title: MISAResource[resource.langCode]?.Dialog?.DeleteDepartment?.Title,
            content: MISAResource[resource.langCode]?.Dialog?.DeleteDepartment?.DeleteContent.replace(
                'DepartmentCode',
                dialog.objectData.DepartmentCode,
            ),
            action: MISAResource[resource.langCode]?.Button?.Delete,
            buttonSec: MISAResource[resource.langCode]?.Button?.No,
            type: Enum.ButtonType.Pri,
...
async function addNewData(data) {
    departmentApi.data = data;
    departmentApi.method = Enum.ApiMethod.POST;
...
async function updateData(data) {
    departmentApi.data = data;
    departmentApi.method = Enum.ApiMethod.PUT;
    const res = await departmentApi.request(`${object.value.DepartmentId}`);
    console.log(res);
    await getData();
...
const exportExcel = async (listIds, search) => {
    try {
        ...
        const excelResquest = {
            ...
            title: MISAResource[resource.langCode]?.Excel?.Department.Title,
            sheet: MISAResource[resource.langCode]?.Excel?.Department.Sheet,
        };
        const res = await departmentApi.downloadExcel(excelResquest);
        ...
        link.href = url;
        link.download = MISAResource[resource.langCode].Excel.Department.FileName;
        link.click();
...
<template lang="">
    <div class="tt-continer__body">
        <div class="tt-continer__header">
            <h3 class="tt-continer__title center">
                {{ MISAResource[resource.langCode]?.Manage?.DepartmentInfo?.Name }}
            </h3>
            <div class="tt-continer__action center">
                ...
                <MISAButton
                    ...
                    :type="Enum.ButtonType.Pri"
                    :action="MISAResource[resource.langCode]?.Button?.AddDepartment"
                >
                    <!-- <template #icon>
        ...
        <div class="tt-continer__table">
            ...
            <div class="tt-continer__filter-table">
                ...
                <div class="tt-continer__filter-right">
                    <MISAGroupInput
                        ...
                        :placeholder="MISAResource[resource.langCode]?.Manage?.DepartmentInfo?.InputSearch"
                        class="width300"
                    ></MISAGroupInput>
                    <MISAButton
            ...
            <div class="tt-continer__table-body">
                <MISATable
                    ...
                    propertiesIdName="DepartmentId"
                    routerCurrent="Department"
    ...
    <Teleport to="#app">
        ...
        <DepartmentForm :buttonLoad="loadingButton" @submit-form="submitFormData" v-if="isShow"></DepartmentForm>
