...
//Input-------------------
const iPositionCode = ref(null);
const iPositionName = ref(null);
...
const getNewCode = async () => {
    try {
        ...
        const res = await positionApi.request('NewCode');
...
const getDataId = async () => {
    try {
        positionApi.method = Enum.ApiMethod.GET;
        if (modalForm.object.PositionId) {
            const res = await positionApi.request(modalForm.object.PositionId);
            return res.data;
        }
        return {};
    } catch (error) {
...
const autoFocusForm = () => {
    try {
        if (validateForm.value.positionCode) {
            iPositionCode.value.autoFocus();
            return;
        }
        if (validateForm.value.positionName) {
            iPositionName.value.autoFocus();
            return;
        }
...
const handleSubmitForm = (isAddNewForm) => {
    try {
        ...
        validateCode(formData.value.positionCode);
        validateName(formData.value.positionName);
...
function validateCode(value) {
    try {
        ...
        validateForm.value.positionCode = errorMessage;
        return errorMessage;
    } catch (error) {
        console.log(error);
    }
...
function validateName(value) {
    try {
        ...
        validateForm.value.positionName = errorMessage;
        return errorMessage;
    } catch (error) {
        console.log(error);
    }
...
function validateDescription(value) {
    try {
        const errorMessage = Validator(value, [
            Validator.maxLength(250, MISAResource[resource.langCode]?.PositionInvalidError?.DescriptionMaxLength),
        ]);
            ...
            <MISARow gutter="12" justify="space-between">
                <MISACol gutter="4" span="10">
                    <MISAInput
                        ...
                        name="id"
                        placeholder="NV-0000"
                        :label="MISAResource[resource.langCode]?.Manage?.PositionInfo?.PositionCode"
                        validate="true"
                        :errorMessage="validateForm.positionCode"
                    ></MISAInput>
                </MISACol>
                <MISACol gutter="4" span="14">
                    <MISAInput
                        ...
                        :errorMessage="validateForm.positionName"
                        require
                        v-model:value="formData.positionName"
                        name="name"
                        :label="MISAResource[resource.langCode]?.Manage?.PositionInfo?.PositionName"
                        validate="true"
                    >
                    </MISAInput>
                </MISACol>
