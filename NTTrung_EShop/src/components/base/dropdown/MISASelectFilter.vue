    ...
    >
        ...
        <div :class="{ 'i-error': errorMessage !== '' }" @focusout="getIndexSelected" class="custom-combobox">
            ...
            <input
                ...
                type="text"
                :value="convertToFilterIcon(inputSearchCombobox)"
            ...
            <div v-show="!props.combobox ? isShowDropdown : true" class="wrapper-select">
                <ul>
                    ...
                    >
                        ...
                        <!-- <img
                            v-if="valueSelected.value === option.value"
...
import { removeVietnameseTones, convertToFilterIcon } from '../../../common/convert-data.js';
...
const props = defineProps({
    ...
    value: { type: [String, Number], default: '' }, //value để binding
...
const updateComboboxInput = (e) => {
    try {
        ...
        if (e.target.value === '') {
            emit('update:value', '');
            emit('blur', '');
            optionsSearchCombobox.value = props.value;
        } else {
            filterOption();
        }
    } catch (error) {
...
const handleKeyDown = (e) => {
    ...
    try {
        ...
        if (e.keyCode === Enum.Keyboard.Enter) {
            ...
            if (indexHoverDropList.value > (props.selectEmpty ? 0 : -1)) {
                inputSearchCombobox.value = optionsSearchCombobox.value[indexHoverDropList.value].value;
...
watch(
    ...
    () => {
        ...
        try {
            inputSearchCombobox.value = valueSelected.value.value;
            filterOption();
            getIndexSelected();
            scrollToOption(true);
        } catch (error) {
